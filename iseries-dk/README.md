...
* `iseries-dk_iopipes`:  DMA-based ASP that supports local memory, host memory, and HSSI/IO Pipes interfaces for the 
kernel system.

* `iseries-dk_usm_iopipes`:  ASP that supports shared virtual memory between host and device. This 
...
Example compilation command: <br>
  `$OFS_ASP_ROOT/$ aoc -v -board=iseries-dk
