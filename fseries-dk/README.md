...
* `fseries-dk_usm`: ASP that supports shared virtual memory between host and device. This 
variant is the same as the non-USM variant with the addition of the USM path between 
the kernel-system and the host.

* `fseries-dk`:  DMA-based ASP that supports local memory and host memory interfaces for the 
kernel system.

* `fseries-dk_iopipes`:  DMA-based ASP that supports local memory, host memory, and HSSI/IO Pipes interfaces for the 
kernel system.

* `fseries-dk_usm_iopipes`:  ASP that supports shared virtual memory between host and device. This 
...
Need to set **OFS_ASP_ROOT** to point to oneapi-asp/fseries-dk.
...
Example compilation command: <br>
  `$OFS_ASP_ROOT/$ aoc -v -board=fseries-dk
